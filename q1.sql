...
DELIMITER / / CREATE PROCEDURE add_full_property(
  in f_uid int(11),
  in f_avail int(1),
...
from PROPERTY p
WHERE p.PID = f_pid;
select p.hike into Mhike
from PROPERTY p
WHERE p.PID = f_pid;
select p.start_date,
  p.end_date into st_date,
  en_date
from PROPERTY p
WHERE p.PID = f_pid;
...
DELIMITER / / CREATE TRIGGER avail_con
AFTER
INSERT ON RENT FOR EACH ROW BEGIN
